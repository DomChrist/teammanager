package de.dom.cishome.myapplication.tm.adapter.`in`.compose.player.components
...
fun TmDatePicker( page1: MutableState<Int>,
                  ...
                  yearRange: Pair<Int,Int> = Pair(LocalDate.now().year - 3 , LocalDate.now().year - 40),
                  ...
                  )
{
    ...
    TmDatePickerView( page1 , date , yearRange = yearRange,onDateSelected=onDateSelected , content = content )
...
fun TmDatePickerView(
    ...
    if( page.value == 0 ){
        chooseYear( yearRange=yearRange, onSelect = {
...
@Composable
fun chooseYear( yearRange: Pair<Int,Int> = Pair(LocalDate.now().year - 3 , LocalDate.now().year - 40),
                onSelect: ( year: Int ) -> Unit
                ){
    var list = mutableListOf<Int>()
    for( i in yearRange.second .. yearRange.first){
        list.add(i);
    }
